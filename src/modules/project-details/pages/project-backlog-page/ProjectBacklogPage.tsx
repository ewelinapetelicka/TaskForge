...
import {
    openDetailsSprint,
    selectSprintDetailOpen,
    selectUndoneSprints
} from "../../../../store/sprints/sprints.slice";
...
import {SprintDetailsDialog} from "../../dialogs/sprint-details-dialog/SprintDetailsDialog";
...
export function ProjectBacklogPage() {
    ...
    return (
        <div className={"pt-2"}>
            <Button label={"ADD NEW SPRINT"} outlined onClick={() => openSprintModal()}></Button>
            {sprints.map((el) => <SprintList sprintId={el.id} key={el.id}></SprintList>)}
            <BacklogList></BacklogList>
            {visible && <SprintDetailsDialog></SprintDetailsDialog>}
        </div>
    )
}
